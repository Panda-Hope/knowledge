...
值得一提的是，我们在compose文件中，新增了network与volume，由于Docker中每个容器之间互相独立，因此我们并不能直接将服务端接口接口与数据库进行链接，
我们构建了本地的docker网络`net`，用于本地局域网通信，同时我们通过volume卷将服务产生的网关日志、服务日志以及数据库数据进行长时间储存，以避免容器重启导致的数据丢失问题。
...
```
在上面的示例中，每次当我们有新的代码推送到master分支，GitHub会自动触发CI流程，开始安装依赖并构建代码镜像，最终将其部署到我们绑定的私人服务器上，完成代码的自动更新操作。

### 服务监测与日志接入
当容器已经在服务器上运行了之后，这个时候对于各个容器的运行状况的监测便是非常重要的事情了，我们要保证自身的服务器时刻处于正常运行状态，
当容器遇到宕机等情况时要及时启用容灾方案，同时也要对网关、服务、数据库的日志进行接入，方便对线上问题进行排查。

<img src="https://github.com/Panda-Hope/panda-hope.github.io/blob/master/static/%E6%88%AA%E5%B1%8F2022-08-17%2011.12.46.png" width="400" />

对于容器的统一管理与容灾，我们通过k8s来进行处理，并将在下一篇k8s篇章继续介绍对于k8s的运用。


## 总结
总得来说，Docker的出现极大了改善运维部署的状况，将以前独立的各个服务器模块，进行了统一的管理，同时Docker的容器化也为容器提供了完全独立化的运行条件，
避免了各个模块之间相互污染~。通过这次的Docker学习，我也将自身的服务部署完全迁移到了Docker，深感Docker对服务管理与维护所带来的便捷。
